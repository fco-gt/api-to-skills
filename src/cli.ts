...
program
  ...
  .requiredOption(
    "--input <path|url>",
    "Path to OpenAPI file, URL, or Markdown file",
  )
  ...
  .option(
    "--enrich",
    "Enable LLM semantic enrichment (requires ANTHROPIC_API_KEY)",
    false,
  )
    ...
    async (opts: {
      ...
      try {
        ...
        let enriched: EnrichedEndpoint[] =
          result.endpoints as unknown as EnrichedEndpoint[];

        if (opts.enrich) {
          const enrichSpinner = ora(
            "Enriching endpoints with Claude...",
          ).start();
          enriched = await enrichWithLlm(result.endpoints);
          enrichSpinner.succeed(
            `Enriched ${chalk.bold(String(enriched.length))} endpoints`,
          );
        } else {
        ...
        console.log("\n" + chalk.bold("═══ Summary ═══"));
        console.log(
          `  Endpoints processed: ${chalk.bold(String(enriched.length))}`,
        );
        console.log(
          `  Skills generated:    ${chalk.bold(String(files.length))}`,
        );
        console.log(
          `  Output directory:    ${chalk.bold(path.resolve(opts.output))}`,
        );
        console.log("");
